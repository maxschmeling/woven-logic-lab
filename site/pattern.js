...
export function patternSVG(model, selected = -1, mode = 'yarn') {
  const width = Math.max(500, 210 + model.outputs.length * 116), height = 110 + model.rows.length * 51;
  const xs = model.outputs.map((_, i) => 180 + i * 116);
  const text = (x, y, content, more = '') => `<text x="${x}" y="${y}" ${more}>${escapeHTML(content)}</text>`;
  let svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" role="img" aria-labelledby="pattern-title pattern-desc"><title id="pattern-title">${mode === 'yarn' ? 'Yarn weaving' : 'Diode connection'} pattern</title><desc id="pattern-desc">${model.rows.length} rows by ${model.outputs.length} outputs. ${mode === 'yarn' ? 'Straight through a ring means one, curved above means zero. This is a human-readable model.' : 'A diode connects each one. Zero crossings stay insulated. Striped cathode connects to row; anode to column.'} A highlighted row shows the selected input.</desc><rect width="${width}" height="${height}" rx="8" fill="#f8f5ed"/><g font-family="ui-monospace,monospace" font-size="11" fill="#595e53">`;
  ...
  model.rows.forEach((row, r) => {
    const y = 71 + r * 51, active = selected === r;
    if (active) svg += `<rect x="7" y="${y - 25}" width="${width - 14}" height="47" rx="5" fill="#dfeade"/>`;
    svg += text(18, y + 4, `${String(r).padStart(2, '0')} / ${row.bits.join('')}`, active ? 'font-weight="bold" fill="#1d5d54"' : '');
    ...
    if (mode === 'yarn') {
      ...
      svg += `<path d="M115 ${y}H${width - 22}" stroke="${color}" stroke-width="3"/>`;
      xs.forEach((x, c) => {
...
export const circuitDetail = `<div class="wiring"><svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 580 205" role="img" aria-label="One bit wiring: sense pin with internal pull-up to diode anode, striped cathode to row wire through a 1k resistor to selected row pin, driven LOW. Inactive row pins are high impedance.">
